...
def currentorder(request):
    currentorder_dict = {}
    p = Order.objects.all(): 
    get_objects = p.filter(is_current=True)
    currentorder_dict[food] = get_objects.order_food
    currentorder_dict[quantity] = get_objects.order_quantity
    table = render_to_string('employee.html', {'currentorder': currentorder_dict})
